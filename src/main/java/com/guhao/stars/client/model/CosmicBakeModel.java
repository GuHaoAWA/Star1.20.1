package com.guhao.stars.client.model;

import com.google.common.collect.ImmutableMap;
import com.guhao.stars.StarsMod;
import com.guhao.stars.api.PerspectiveModelState;
...
public final class CosmicBakeModel implements BakedModel {
    ...
    public void renderItem(ItemStack stack, ItemDisplayContext transformType, PoseStack pStack, MultiBufferSource buffers, int packedLight, int packedOverlay) {
        ...
        RenderType renderType = StarShaders.COSMIC_RENDER_TYPE;
        this.parentState = TransformUtils.stateFromItemTransforms(wrapped.getTransforms());
        ...
        float camPitch = 0.0F;
        float scale = 1.0F;
        boolean isGUIMode = StarShaders.inventoryRender || transformType == ItemDisplayContext.GUI;
        boolean isStarrySkyShader = (renderType == StarShaders.SKY_ITEM);
        boolean isCosmicShader = (renderType == StarShaders.COSMIC_RENDER_TYPE || renderType == StarShaders.COSMIC_BLOCK_RENDER_TYPE);
        ...
        if (isStarrySkyShader && isGUIMode) {
            renderType = StarShaders.SKY_ITEM_GUI;
        }

        StarShaders.cosmicTime.set((System.currentTimeMillis() - StarShaders.renderTime) / 2000.0F);
        StarShaders.cosmicYaw.set(yaw);
        StarShaders.cosmicPitch.set(pitch);

        if (StarShaders.camYaw != null && StarShaders.camPitch != null) {
            StarShaders.camYaw.set(camYaw);
            StarShaders.camPitch.set(camPitch);
        }

        StarShaders.cosmicExternalScale.set(scale);
        StarShaders.cosmicOpacity.set(1.0F);
        for (int i = 0; i < 10; ++i) {
            TextureAtlasSprite sprite = Minecraft.getInstance().getTextureAtlas(InventoryMenu.BLOCK_ATLAS).apply(StarsMod.path("item/cosmic_" + i)); //  at assets.adorable-armory.textures.item
            StarShaders.COSMIC_UVS[i * 4] = sprite.getU0();
            StarShaders.COSMIC_UVS[i * 4 + 1] = sprite.getV0();
            StarShaders.COSMIC_UVS[i * 4 + 2] = sprite.getU1();
            StarShaders.COSMIC_UVS[i * 4 + 3] = sprite.getV1();
        }

        StarShaders.cosmicUVs.set(StarShaders.COSMIC_UVS);
            ...
            for (TextureAtlasSprite sprite : atlasSprite) {
                ...
                for (BlockElement element : unbaked) {
                    for (Map.Entry<Direction, BlockElementFace> entry : element.faces.entrySet()) {
                        quads.add(FACE_BAKERY.bakeQuad(element.from, element.to, entry.getValue(), sprite, entry.getKey(), new PerspectiveModelState(ImmutableMap.of()), element.rotation, element.shade, StarsMod.path("dynamic")));
                    }
                }
            }
